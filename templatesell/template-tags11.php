...
if ( ! function_exists( 'polite_posted_by' ) ) :
	...
	function polite_posted_by() {
		$byline = sprintf(
			...
			esc_html_x( '%s', 'post author', 'polite' ),
			'<span class="author vcard"><a class="url fn n" href="' . esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ) . '">' . esc_html( get_the_author() ) . '</a></span>'
		);
...
if ( ! function_exists( 'polite_entry_meta' ) ) :
	...
	if ( 'post' === get_post_type() ) {
		...
		if ( $categories_list ) {
			printf( '<span class="cat-links">' . '' . esc_html__( '%1$s', 'polite' ) . '</span>', $categories_list ); // WPCS: XSS OK.
		}

		/* translators: used between list items, there is a space after the comma */
		$tags_list = get_the_tag_list( '', esc_html__( ', ', 'polite' ) );
		if ( $tags_list && is_singular() ) {
			printf( '<span class="tags-links">' . '<i class="fa fa-tag"></i>' . esc_html__( '%1$s', 'polite' ) . '</span>', $tags_list ); // WPCS: XSS OK.
		}
